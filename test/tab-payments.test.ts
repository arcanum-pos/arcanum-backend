...
describe('tab payments: charge rules', () => {
  it('refuses a charge for a different amount than outstanding (409), even a deliberate partial one above it', async () => {
    const org = await seedOrg();
    const tab = await openTab(org);
    expect((await chargeCash(org, tab.id, 999)).status).toBe(409);
    expect((await chargeCash(org, tab.id, 1151, { partial: true })).status).toBe(409);
    expect((await chargeCash(org, tab.id, 0, { partial: true })).status).not.toBe(201);
  });
